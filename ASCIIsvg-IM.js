...
var showYaxis = 1;
var doGrids = 1;
var boardWidth, boardHeight, boardWidthToHeight=0.75, boardLeft, boardTop;
...
function arc(start,end,radius,id) { // coordinates in cartesian units
  var node, v;
  if (id!=null) node = doc.getElementById(id);
  if (radius==null) {
...
function makeDraggable(targ, func, curveId) {
...
function Drag(evt) {
	...
	if ( (!drag || !dragging) &&  (et == "mousedown" || et == "touchstart" )) {
		...
		if ('function' === typeof window.doFirstOnElementSelect) {
		...
		if ('function' === typeof window.doAfterElementSelect) {
		...
		if(mseTchPosi.x > 4 && mseTchPosi.x < boardPropsArr[slicedSVG+"Width"] - 4 
			...
			if(Math.abs(drag._x) > 0 || Math.abs(drag._y) > 0) {
				...
				if(targetId.length > 0 && targetId.indexOf("board") == -1) {
					...
					if ('function' === typeof window.doOnElementDrag) {
						doOnElementDrag();
					}						
				}				
			}
	...
	if (drag && (et == "mouseup"  )) {
		drag = "undefined";		
		dragging = false;
		if ('function' === typeof window.doOnDeselectElement) {
			doOnDeselectElement();
		}			
	}
	...
	if (drag && (et == "mouseout")) {
		...
		drag = null;
		dragging = false;
		if ('function' === typeof window.doOnMouseOut) {
			doOnMouseOut();
		}	
	}	
}	 
...
//console.log(strokewidth)		
	...
	//text(center, id, (pos == null ? "below" : pos), (id == null ? id : id + "label"))
...
function draggablePtsSegLineJoiner(lineSeg, lineSegType, ptsArray, ptNamesArr ) {
	brdPropsArr[brdID][lineSeg] = ptNamesArr;
	strokewidth = segstrokewidth;
	if(lineSegType == "segment") {
		segment( ptsArray[0], ptsArray[1], lineSeg);
	} else {
		line( ptsArray[0], ptsArray[1], lineSeg);	
	}
	// So segment or line is on top of all other elements
	gebi(svgID).appendChild(gebi(lineSeg));
	strokewidth = dotstrokewidth;
	ASdot(ptsArray[0], dotradius, stroke, fill, ptNamesArr[0]);
	ASdot(ptsArray[1], dotradius, stroke, fill, ptNamesArr[1]);	
	...
	makeDraggable( gebi(ptNamesArr[0]));	
	makeDraggable( gebi(ptNamesArr[1]) );	
}	
...
function segment(p, q, id, strokedasharray) {
//console.log(p,q)	
	setBoardParams(svgID);
		...
		if (marker == "dotdot" || marker == "dotarrow" || marker == "dot" || marker == "arrowdot") {
			...
			if (marker == "dotarrow" || marker == "arrowdot") {
				if (marker == "dotarrow") {
					...
					arrowhead(p, [q[0]-(dotSpacerX)/xunitlength, q[1]-(dotSpacerY)/yunitlength]);
				}
			}
...
function plot(fun, x_min, x_max, points, id) {
	...
	if (typeof fun == "string" && fun.indexOf("y") > -1) {
		...
		// CASE 1: Goes outside upper or lower graph limits - break and send to path() - give idInc extension for id
...
//console.log("id="+typeof(id))	
					if(typeof(id) == "string") {
						...
						// Subsequent ids are generated by path() fn.
						path(pth, id);
						id = null;
					} else {
						path(pth);
...
//console.log("hyar: "+id, p, str);	
	...
	//
	/////////////////////////////////////////
	strTxt = strip_tags(str); // Test for HTML tags or entities
...
//	if (typeof dy=="string") { gdx = dy; dy = null; }
	...
	if(doAxes == 1) {
		...
		} //syntax: text(p,str,pos,id,fontsty,fontfam)... Complains if id set
		...
		text([xmax+(padding-10)/xunitlength,0],xAxisVble,"above",svgID+"xAxVbl","",'KaTeX_Math,"Times New Roman",Times,serif'); // x-axis label
		if( showYaxis == 1) {
			text([0,ymax+(padding-10)/yunitlength],yAxisVble,"right",svgID+"yAxVbl","",'KaTeX_Math,"Times New Roman",Times,serif'); // y-axis label
		}
...
function initBoard(divID, x_min,x_max,y_min,y_max){
